...
def alternative_sample(states_num: int = 4,
    ...
    If unspecified, the default state number
    is 4, noise_scale is 0.01 and seed is 2021.
    The transition probability matrix will hold
    a default type of None, which then will be
    randomly generated by the np.random.default_rng
    and softmax function to a matrix of size
    states_num x states_num.
